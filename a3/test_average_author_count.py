...
def test_multiple_articles() -> None:
    """Test average_author_count with TEST_ARXIV."""
    actual = arxiv_functions.average_author_count(TEST_ARXIV)
    expected = 1.6
    assert actual == pytest.approx(expected)

def test_empty_dict() -> None:
    """Test average_author_count with empty dict."""
    actual = arxiv_functions.average_author_count({})
    expected = 0.0
    assert actual == expected

def test_one_article() -> None:
    """Test average_author_count with one article in the dict."""
    test = {'108': {
        ID: '108',
        TITLE: 'CSC108 is the Best Course Ever',
        CREATED: '2023-09-01',
    ...
    actual = arxiv_functions.average_author_count(test)
    expected = 3.0
    assert actual == expected

def test_returning_float() -> None:
    """Test average_author_count when the count of authors cannot be 
    evenly divided."""
    test = {
    '5090': {
        ID: '5090',
...
def test_arxivtype_mutation() -> None:
    """Comfirm average_author_count does not mutate the dict given."""
    actual = TEST_ARXIV
    expected = TEST_ARXIV
    arxiv_functions.average_author_count(actual)
    assert actual == expected
